...
class SearchViewModel: ObservableObject {
    ...
    private func SearchSongRequest(key: String, type: NCMSearchType) {
        NCM.requestPublisher(action: SearchSongAction(.init(s: key, type: type, limit: limit, offset: offset * limit)))
            .sink { completion in
            ...
            weakSelf.result = .init(songs: result.songs?.map(NeteaseSong.init) ?? [], hasMore: result.hasMore ?? false)
        }
